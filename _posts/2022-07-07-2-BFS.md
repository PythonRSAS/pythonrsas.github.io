...
2. while anything is in queue, pop the first item from the queue, (it is now visited and is black) 
3. if what's popped out has not been visited yet, add it to the <span class="coding">visited</span> is a set instead of a list. 
...
BFS works on both directed and undirected graph.  
```python
graph = {
    'A': ['B', 'C'],
    'B': ['A', 'D', 'E', 'F'],
...
> The negative side of undirected graph is that we have to do more membership tests.  For example, when we are at the $$B$$ node, we have to ask if $$A$$ was in the <span class="coding">visited</span> even though we just came from A. 
